...
export function printA4Receipt(job, customer, repair, bill, baseUrl) {
  ...
  w.document.write(`<!DOCTYPE html><html><head>
    ...
    <div class="head">
      <div style="display:flex;align-items:center;gap:10px">
        <img src="${assetUrl('/logo.png')}" alt="" style="width:32px;height:32px;border-radius:6px;object-fit:contain"/>
    ...
    <div class="body">
      ...
      <div class="ft">
        ...
        <div class="qr">
          <img src="${assetUrl('/track/qr?url=')}${encodeURIComponent(trackingUrl)}" alt="QR"/>
          <div class="l">Scan to track repair</div>
        </div>
      </div>
      <div class="sign">
        <svg width="56" height="22" viewBox="0 0 120 30"><text x="5" y="22" font-family="cursive" font-size="24" fill="#555">${(customer?.name||'C').charAt(0)}</text></svg>
...
export function printThermalLabel(job, customer, baseUrl) {
  const w = window.open('', '_blank');
  if (!w) return;
...
@media print{@page{margin:0}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .left .qr .fb{width:19mm;height:19mm;display:flex;align-items:center;justify-content:center;font-size:5px;color:#999;background:#f5f5f5;border-radius:1mm}
      .left .ql{font-size:4px;color:#888;text-align:center;line-height:1}
      .right{flex:1;display:flex;flex-direction:column;overflow:hidden;padding-left:0.5mm;padding-top:7.5mm;padding-bottom:0.5mm}
      ...
      .right .bc .fb{font-size:6px;font-weight:700;font-family:monospace;word-break:break-all;line-height:1;color:#333}
      .right .jid{font-size:5px;color:#666;line-height:1.1;font-family:monospace}
    </style></head><body>
    <div class="row">
      <div class="left">
        <div class="qr"><img src="${assetUrl('/track/qr?url=')}${encodeURIComponent(trackingUrl)}" alt="QR" onerror="this.parentNode.innerHTML='<div class=fb>QR</div>'"/></div>
        <div class="ql">Scan to track repair</div>
      </div>
      <div class="right">
    ...
    try{
      ...
      document.querySelector('.right .bc').innerHTML='<div class=fb>${job.jobId}</div>';
    }
